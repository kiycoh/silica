...
# ---------------------------------------------------------------------------
# build_segmented_context — static/dynamic profile split
# ---------------------------------------------------------------------------

from silica.kernel.context_builder import build_segmented_context


def test_segmented_static_profile_appears_first():
    result = build_segmented_context(
        "distill",
        static_profile="vault style: Italian, linked thinking",
        ledger_digest="RUN 00000001",
    )
    assert result.index("vault style") < result.index("RUN 00000001")


def test_segmented_static_profile_section_label():
    result = build_segmented_context("distill", static_profile="some profile text")
    assert "Vault Profile" in result
    assert "some profile text" in result


def test_segmented_no_static_omits_vault_profile_section():
    result = build_segmented_context("distill", payload="PAYLOAD")
    assert "Vault Profile" not in result


def test_segmented_dynamic_sections_present():
    result = build_segmented_context(
        "distill",
        payload={"x": 1},
...
def test_segmented_static_before_dynamic_before_payload():
    result = build_segmented_context(
        "distill",
        payload="PAYLOAD",
        ledger_digest="DIGEST",
    ...
    assert result.index("STATIC") < result.index("DIGEST")
    assert result.index("DIGEST") < result.index("SUBSTRATE")
    assert result.index("SUBSTRATE") < result.index("PAYLOAD")


def test_segmented_empty_static_omitted():
    result = build_segmented_context("distill", payload="P", static_profile="   ")
    assert "Vault Profile" not in result


def test_segmented_pure_dynamic_matches_build_context():
    from silica.kernel.context_builder import build_context
    result_seg = build_segmented_context(
        "distill", payload={"a": 1}, ledger_digest="D", substrate="S"
    )
    result_plain = build_context(
        "distill", payload={"a": 1}, ledger_digest="D", substrate="S"
    )
    assert result_seg == result_plain
